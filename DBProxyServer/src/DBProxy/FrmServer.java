...
import DBPluginManager.PluginManager;
	...
	public FrmServer() {
		...
		btn_Start.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent arg0) {
			    JsonConfig rdConfig=new JsonConfig();
			    rdConfig.GlobalRead();
				  ServerProxy = new RemoteProxy();
				  ServerProxy.init(GlobalConfig.LocalAddr);
				    DefaultTableModel model=(DefaultTableModel) table.getModel();
					Object[] data=new Object[]{new UILocalDate(new Date()),"一般","启动服务"};
					model.addRow(data);
					data=new Object[]{new UILocalDate(new Date()),"一般","服务地址："+GlobalConfig.LocalAddr};
                    model.addRow(data);
                    //
                    PluginManager.getInstance().startAllPlugin("", 1);
			}
		});
	...
	public void AddLog(Object content)
	{
		...
		if(info!=null)
		{
			...
			if(info.logType!=LogType.Test)
			{
				...
				DefaultTableModel model=(DefaultTableModel) table.getModel();
				Object[] data=new Object[]{new UILocalDate(info.logTime),info.logType,info.logMsg};
